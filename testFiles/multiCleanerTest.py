...
def cleanHyperlinks(badWorksheet, startIndex, newID):
    basePath = fr'\\ucclerk\pgmdoc\Veterans\test'
    fileDirectoryMap = {}
...
def compareHyperlinkLetters(workbookPath):
    ...
    pattern = re.compile(
        r'test - Redacted[\\/][^\\/]+ - Redacted[\\/](?P<fileLetter>[A-Z])[\\/][^\\/]+(?P<fileLetter>[A-Z])\d+ redacted\.pdf'
    )
...
def main(goodIDs, badIDs):
    excelFilePath = r"\\ucclerk\pgmdoc\Veterans\VeteransTest.xlsx"
    goodWorkSheet = None
    badWorksheet = None
