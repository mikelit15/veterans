...
def mergeImages(pathA, pathB, cemetery, letter):
    ...
    with open(f'{fullLocation}\\{cemetery}{letter}{fileName[-1].replace("a.pdf", "")} redacted.pdf', 'wb') as outputPDF:
...
def analyzeDocument(filePath, id, suffix):
    ...
    poller = documentAnalysisClient.beginAnalyzeDocument("Test2.4n", document=bytesTest)
...
def main(singleFlag, singleCem, singleLetter):
    ...
    workbook = openpyxl.load_workbook('VeteransTest.xlsx')
    cemetery = singleCem 
    ...
    for y in range(len(pdfFiles)):
        ...
        with open(logFilePath, 'w', encoding='utf-8') as logFile:
            ...
            logFile.write("\n")
        workbook.save('VeteransTest.xlsx')
        if breakFlag:
            break
